...
class InfluenceBalancedLoss(CrossEntropy):
    ...
    def forward(self, inputs, targets, **kwargs):
        ...
        grads = torch.sum(torch.abs(F.softmax(inputs, dim=1) - F.one_hot(targets.to(torch.int64), self.num_classes)), 1)
        ib = grads * feature.reshape(-1)
        ib = self.alpha / (ib + 1e-3)
        ib_loss = F.cross_entropy(inputs, targets.long(), reduction='none', weight=self.weight_list)*ib
        return ib_loss.mean()
